export default async function getDocumentAnalytics(documentId: string) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_POSTHOG_HOST}/api/projects/${process.env.NEXT_PUBLIC_POSTHOG_PROJECT_ID}/events/?event=Documentview&properties=[{"key":"document_id","value":"${documentId}","operator":"exact"}]`,
    {
      headers: {
